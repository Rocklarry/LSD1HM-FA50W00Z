...
{
    GTP_DEBUG("gtp_touch_down enter");
#if GTP_CHANGE_X2Y
    GTP_SWAP(x, y);
#endif
#if 0
    ...
    input_report_abs(ts->input_dev, ABS_PRESSURE, 1000);		
...
#endif
	input_report_key(ts->input_dev, BTN_TOUCH, 1);
        input_report_abs(ts->input_dev, ABS_X, abx);
        input_report_abs(ts->input_dev, ABS_Y, aby);
...
#endif
	input_set_abs_params(ts->pen_dev, ABS_X, 0, 0x400, 0, 0);
   input_set_abs_params(ts->pen_dev, ABS_Y, 0, 0x400, 0, 0);    
	input_set_abs_params(ts->pen_dev, ABS_PRESSURE, 0, 1000, 0, 0);
...
static void gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
    ...
    input_report_abs(ts->pen_dev, ABS_PRESSURE, 1000);
...
#endif
	input_report_key(ts->pen_dev, BTN_TOUCH, 0);
	input_report_abs(ts->pen_dev, ABS_X, abx);
	input_report_abs(ts->pen_dev, ABS_Y, aby);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)
        {
            ...
            {
		input_report_key(ts->input_dev, BTN_TOUCH, 1);
    input_report_abs(ts->input_dev, ABS_X,  input_x+1);
    input_report_abs(ts->input_dev, ABS_Y,  input_y+1);
    input_report_abs(ts->input_dev, ABS_PRESSURE, 1000);
    input_sync(ts->input_dev);	
                gtp_touch_down(ts, id, input_x, input_y, input_w);
            }
...
#endif
	 input_set_abs_params(ts->input_dev, ABS_X, 0, 0x400, 0, 0);
   input_set_abs_params(ts->input_dev, ABS_Y, 0, 0x400, 0, 0);
        input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 1000, 0, 0);
