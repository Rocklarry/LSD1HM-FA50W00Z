...
static int sunxi_audio_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	snd_soc_dapm_disable_pin(&runtime->card->dapm,	"Phoneout speaker");
		...
		struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_I2S | (ctx->aif2fmt << 8) | (ctx->aif2master << 12));
	if (ret < 0) {
...
static int __devinit sunxi_sndpcm_dev_probe(struct platform_device *pdev)
{
	...
	script_item_u usb_item;
	script_item_u led_item;
	...
	type = script_get_item("audio0", "usb_detect", &usb_item);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		usb_item.gpio.gpio = -1;
		pr_err("[ audio ] err:try to get usb_detect failed!\n");
	} else {
		ret = gpio_request(usb_item.gpio.gpio, NULL);
		if (0 != ret) {
			pr_err("request gpio PH09 failed!\n");
		} else {
			if (gpio_is_valid(usb_item.gpio.gpio)) {
				gpio_direction_output(usb_item.gpio.gpio, 1);
				/* gpio_set_value(usb_item.gpio.gpio, 1); */
			} else
				pr_err("gpio is not valid!\n");
		}
	}

	type = script_get_item("audio0", "mic_array_led_ctrl", &led_item);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		led_item.gpio.gpio = -1;
		pr_err("[ audio ] err:try to get mic_array_led_ctrl failed!\n");
	} else {
		ret = gpio_request(led_item.gpio.gpio, NULL);
		if (0 != ret) {
			pr_err("request gpio PH06 failed!\n");
		} else {
			if (gpio_is_valid(led_item.gpio.gpio)) {
				gpio_direction_output(led_item.gpio.gpio, 1);
				/* gpio_set_value(led_item.gpio.gpio, 1); */
			} else
				pr_err("gpio is not valid!\n");
		}
	}
